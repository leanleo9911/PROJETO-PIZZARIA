...
function escolherOpcao(lista: string[], texto: string): string {
    ...
    while (true) {
        const escolha = readlineSync.question('Escolha o número: ');
...
function escolherFormaPagamento(): string {
    const opcoes = ['Dinheiro', 'Cartão de Crédito/Débito', 'Pix'];
...
export function registrarPedido(): void {
    ...
    const itensPedido: [string, number, number][] = [];
    let valorTotal = 0;
    ...
    console.log(`✅ Comprovante salvo na área de trabalho: ${nomeArquivo}`);
}
...
function consultarOuAtualizarVenda(): void {
    ...
    const escolha = readlineSync.question('\nDigite o número da venda para ver detalhes ou ENTER para voltar: ').trim();
