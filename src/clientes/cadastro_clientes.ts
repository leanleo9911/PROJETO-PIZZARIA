...
function salvarClientes(clientes: any[]): void {
...
export function cadastrarCliente(): void {
    ...
    while (true) {
        ...
        console.log('❌ E-mail inválido. Exemplo: usuario@dominio.com');
    }
    ...
    while (true) {
        ...
        console.log('❌ Telefone inválido. Use apenas números, espaços, parênteses e traços.');
    }

    const endereco = readlineSync.question('Digite o Endereço (Ex: Rua Exemplo, 123): ');

    let cep: string;
    while (true) {
        cep = readlineSync.question('CEP (Ex: 00000-000): ');
        if (validarCep(cep)) break;
        console.log('❌ CEP inválido. Use o formato 00000-000.');
    }
...
function consultarOuAtualizarCliente(): void {
    ...
    console.log('\n--- Lista de Clientes ---');
    clientes.forEach(c =>
        console.log(`ID: ${c.id} | Nome: ${c.nome} | Email: ${c.email} | Telefone: ${c.telefone}`)
    );
...
function excluirCliente(): void {
    ...
    console.log('✅ Cliente excluído e IDs renumeradas com sucesso!');
}
